...
/**
 * The main class for the Orbital Motion Simulator application.
 * The main method here calls the JavaFX application's main method. This class
 * is useful if we'd like to run the application via command line, since
 * otherwise it is cumbersome to do so with a JavaFX application.
 * 
...
public class Main {
    ...
    private static void testSatelliteCalculation() {
        ...
        System.out.printf("Initial Position: (%.3e, %.3e, %.3e) m%n", data.initialPosition.x, data.initialPosition.y,
                data.initialPosition.z);
        System.out.printf("Initial Velocity: (%.3e, %.3e, %.3e) m/s%n", data.initialVelocity.x, data.initialVelocity.y,
        ...
        System.out.printf("Angular Momentum Vector: (%.3e, %.3e, %.3e)%n", data.angularMomentumVect.x,
                data.angularMomentumVect.y, data.angularMomentumVect.z);
        System.out.printf("Eccentricity Vector: (%.3e, %.3e, %.3e)%n", data.eccentricityVect.x, data.eccentricityVect.y,
                data.eccentricityVect.z);
        ...
        System.out.printf("Periapsis radius: %.3e m (%.2f km)%n", data.radiusOfPeriapsis,
                data.radiusOfPeriapsis / 1000);
        ...
        System.out.printf("Line of Nodes Vector: (%.3e, %.3e, %.3e)%n", data.lineOfNodesVect.x, data.lineOfNodesVect.y,
                data.lineOfNodesVect.z);
        System.out.printf("Longitude of Ascending Node: %.6f rad (%.2f deg)%n", data.longitudeOfAscendingNode,
                Math.toDegrees(data.longitudeOfAscendingNode));
        System.out.printf("Inclination: %.6f rad (%.2f deg)%n", data.inclination, Math.toDegrees(data.inclination));
        System.out.printf("Argument of Periapsis: %.6f rad (%.2f deg)%n", data.argumentOfPeriapsis,
                Math.toDegrees(data.argumentOfPeriapsis));
        System.out.printf("Initial True Anomaly: %.6f rad (%.2f deg)%n", data.initialTrueAnomaly,
                Math.toDegrees(data.initialTrueAnomaly));
        ...
        // Need to wait a bit before getting the first values because of initialization
        // time of the threads, otherwise the first values will be 0 and it will cause a
        // rejection in the getRelativeInfo method of the satellite
        try {
            Thread.sleep(1000);
